...
//!
//! Orchestration only, and it lives in the service because the service owns the
//! hardware. The editor asks for a takeover over the pipe; it never performs one.
//!
...
pub fn survey(engine: &EngineHandle) -> ContentionReport {
    let labels: std::collections::BTreeMap<String, String> = engine
        .inventory()
        .channels
        .into_iter()
        .map(|c| (c.id, c.label))
    ...
    let controls = engine.channel_controls();
...
pub fn take_over(engine: &EngineHandle, force: bool) -> TakeoverResult {
    let mut steps = Vec::new();

    let before = survey(engine);
    if before.clear {
    ...
    let stranded = survey(engine).stranded;
    if stranded.is_empty() {
        steps.push("Every channel is accounted for; nothing needed handing back.".to_owned());
    } else {
        for (id, outcome) in engine.hand_back_to_firmware(stranded) {
