...
//!
//! Owns the tick loop, the hardware backends, and — most importantly — the safety
//! supervisor. The layered safety design is specified in `plans/open-fan.md`; this crate
//! is where layers 1 through 6 are implemented.
//!
//! # Status
//!
//! Phase 1 scaffold. What exists now is the piece that has to be right before anything
//! else is built on it: the **failsafe policy** and the rule that turns a graph tick into
//! hardware writes. The tick loop, watchdog thread, external watchdog process and crash
//! reporting land in Phase 4.
