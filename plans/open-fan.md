...
- [ ] **Phase 2 — Graph core.** ← *current step*
      Finish the node catalogue (hysteresis, low-pass, PID, comparator, switch, latch,
      schedule, process-state sources), wire the editor to the backend document, start the
      engine tick loop, stream live readouts to the UI. The type system, evaluator,
      `of-hal` traits and `of-hal-mock` plant already exist from Phase 1.
